...
with open("visualize/comparison.tex", "w") as outFile:
   ...
   for i in range(len(real)):
       color = {"Voice,TAM" : None, "Valence" : "orange", "Voice" : "green", "TAM" : "blue", "Agreement" : "purple", "NA" : None}[alignment[real[i]]]
       print("\\node[rectangle,text width=1.2cm,anchor=base"+(", fill="+color+"!20" if color is not None else "") + "] (A"+str(i+1)+") at (1,"+str(-i/2.0-1)+") {"+slotNames[real[i]]+"};", file=outFile)
   for i in range(len(optimized)):
       color = {"Voice,TAM" : None, "Valence" : "orange", "Voice" : "green", "TAM" : "blue", "Agreement" : "purple", "NA" : None}[alignment[optimized[i]]]
       print("\\node[rectangle,text width=1.2cm,anchor=base"+(", fill="+color+"!20" if color is not None else "") + "] (B"+str(i+1)+") at (4,"+str(-i/2.0-1)+") {"+slotNames[optimized[i]]+"};", file=outFile)
   for i in range(len(optimized)):
