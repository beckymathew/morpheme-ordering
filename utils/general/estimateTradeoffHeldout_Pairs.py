...
def calculateMemorySurprisalTradeoff(train, dev, args, getSurprisalRepresentation, extractRepresentationForAveragingMI):
    """ Input: 
    ...
    dev = [getSurprisalRepresentation(x) for x in dev_raw]
    train = [getSurprisalRepresentation(x) for x in train_raw]

    devForPMI = [extractRepresentationForAveragingMI(x) for x in dev_raw]

    # idev and itrain are suffix arrays: They contain indices to positions in the text, ordered by the alphabetical position of the suffixes starting at them ["suffix" is used here in the sense of "suffix of the entire corpus", not an individual linguistic morpheme].
    idev = range(len(dev))
    itrain = range(len(train))
   
    # This sorts the arrays by the alphabetical position of the suffixes
...
#       print(k)
    ...
    #      print(dev[idev[j]])
          ...
          devForPMIPref = tuple(devForPMI[idev[j]:idev[j]+k+1])
       ...
       #      print("PMI contribution", lastProbability[j]-newProbability[j], devPref[0], devPref[-1])
             pmis[(devForPMIPref[-1], devForPMIPref[0])].append(-lastProbability[j]+newProbability[j])
          elif k == 0:
