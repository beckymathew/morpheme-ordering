    ...
    if len(verb) > 0:
      ...
      for group in verb:
         for morpheme in zip(group["posFine"].split("+"), group["lemma"].split("+")):
           ...
           flattened.append({"kaist_label": fine_label, "grapheme": morph})
      ...
      for item in flattened: 
        if item["kaist_label"][0] == "n" and not item["kaist_label"][:3] == "nbn":
          if len(joined_nouns) > 0:
            if joined_nouns[-1]["kaist_label"][0] == "n" and not joined_nouns[-1]["kaist_label"][:3] == "nbn":
              joined_nouns[-1]["kaist_label"] += "_" + item["kaist_label"]
              joined_nouns[-1]["grapheme"] += "_" + item["grapheme"]
        else:
