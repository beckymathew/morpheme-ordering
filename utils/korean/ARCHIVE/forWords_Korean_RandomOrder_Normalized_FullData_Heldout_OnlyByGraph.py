...
#       flattened = []
#       for group in verb:
#          for fine_label, morpheme in zip(group["posFine"].split("+"), group["lemma"].split("+")):
#            morph, fine_label = allomorphy.get_underlying_morph(morpheme, fine_label)
#            flattened.append(morph)
...
def processVerb(verb, data_):
    if len(verb) > 0:
      ...
      # make verb lists only in Hangul graphemes, no labels
      for lst in lsts: 
        for i, item in enumerate(lst):
          submorphs = item.split("_")
          hangul = ""
          for j in range(1, len(submorphs), 2): # only get the Hangul morphs, not the labels
            hangul += submorphs[j]
          lst[i] = hangul 
