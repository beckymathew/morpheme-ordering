 ...
 #             print(morph_label)
  ...
  #            print("107", x, fine_label, morph)
   ...
   #                  print(z)
          ...
          data.append({"parsed" : lst2, "original" : verb})
    #      print(lst2)
...
for d in data:
   affixChains[tuple(d["parsed"][1:])].append(d["original"])

for x in sorted(list(affixChains), key=lambda x:len(affixChains[x]))[-50:]:
    ...
    print([y for y in chains if y[0] == chainsCount[0][0]][0])
    print([y for y in chains if y[0] == chainsCount[1][0]][0])
    print([y for y in chains if y[0] == chainsCount[2][0]][0])


for x in sorted(list(affixChains), key=lambda x:len(affixChains[x]))[-50:][::-1]:
    chains = [("&".join([y["word"] for y in z]), z) for z in affixChains[x]]
    chainsCount = defaultdict(int)
    for z, _ in chains:
        chainsCount[z] += 1
    chainsCount = sorted(list(chainsCount.items()), key=lambda x:x[1], reverse=True)
    example = [y for y in chains if y[0] == chainsCount[0][0]][0]
    print("\t&\t".join(["\\korean{"+example[0]+"}", "\\korean{"+example[1][0]["lemma"]+"}", example[1][0]["posFine"], "+".join([y["fine"].replace("_", "-").replace("하", "\\korean{하}").replace("되", "\\korean{되}").replace("ㄴ다면", "\\korean{ㄴ다면}").replace("자", "\\korean{자}") for y in x]), str(len(affixChains[x]))])+" \\\\")


print(list(affixChains)[:5])
for x in sorted([i for i in list(affixChains) if len(i) >0 and (len(i) > 1 or "CONNECTOR" not in i[0]["fine"])], key=lambda x:len(affixChains[x]))[-50:][::-1]:
    chains = [("&".join([y["word"] for y in z]), z) for z in affixChains[x]]
    chainsCount = defaultdict(int)
    for z, _ in chains:
        chainsCount[z] += 1
...
for x in sorted([i for i in list(affixChains) if len(i) >0 and (len(i) > 1 or "CONNECTOR" not in i[0]["fine"]) and ("DERIVATION" not in i[0]["fine"])], key=lambda x:len(affixChains[x]))[-50:][::-1]:
    chains = [("&".join([y["word"] for y in z]), z) for z in affixChains[x]]
    chainsCount = defaultdict(int)
    for z, _ in chains:
        chainsCount[z] += 1
