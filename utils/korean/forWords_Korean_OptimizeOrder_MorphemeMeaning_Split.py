...
from korean_morpheme_meanings import automatic_morpheme_meaning
...
def calculateTradeoffForWeights(weights):
    ...
    for data, processed in [(data_train, train), (data_dev, dev)]:
      for verb in data:
         affixes = verb[1:]
        #  affixes = sorted(affixes, key=lambda x:weights.get(getRepresentation(x), 0)) # morpheme meaning slots representation
         affixes = sorted(affixes, key=lambda x: sum(weights.get(slot, 0) for slot in getRepresentation(x)))
