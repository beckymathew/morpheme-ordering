...
names(data) <- c("POS", "Language", "Type", "Accuracy")
...
plot = plot + geom_segment(data=means %>% filter(POS == "Nouns", Type == "Baseline"), aes(x=0.5, xend=1.5, y=Accuracy, yend=Accuracy), size=1)
...
plot = plot + geom_segment(data=means %>% filter(POS == "Verbs", Type == "Baseline"), aes(x=0.5, xend=1.5, y=Accuracy, yend=Accuracy), size=1)
