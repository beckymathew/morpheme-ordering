...
from estimateTradeoffHeldout_ByElement import calculateMemorySurprisalTradeoff
...
def calculateTradeoffForWeights(weights):
    ...
    auc, devSurprisalTable, MIs = calculateMemorySurprisalTradeoff(train, dev, args)
    return MIs
...
for length, x in pmis:
   pmis_coarse[(length, coarse(x))] += pmis[(length,x)]

with open(f"estimates_byWord/{__file__}_{args.language}_{args.model.split('_')[-1]}", "w") as outFile:
  for length, x in sorted(list(pmis_coarse), key=lambda x:x[::-1]):
   print("\t".join([str(q) for q in [length, x, mean(pmis_coarse[(length, x)])]]), file=outFile)
