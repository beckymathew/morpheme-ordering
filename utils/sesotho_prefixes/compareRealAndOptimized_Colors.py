...
slotNames = {"Object/reflexive" : "Object", "Int/Rel" : "NA", "Tense/aspect" : "Tense/Aspect", "Infinitive" : "NA"}
...
with open("visualize/comparison.tex", "w") as outFile:
   ...
   for i in range(len(real)):
       ...
       print("\\node[rectangle,text width=2.0cm,anchor=base"+(", fill="+color+"!20" if color is not None else "") + "] (A"+str(i+1)+") at (1,"+str(-i/2.0-1)+") {"+real[i]+"};", file=outFile)
   for i in range(len(optimized)):
       color = {"Valence" : "orange", "Voice" : "green", "TAM" : "blue", "Agreement" : "purple", "NA" : None}[alignment[optimized[i]]]
       print("\\node[rectangle,text width=2.0cm,anchor=base"+(", fill="+color+"!20" if color is not None else "") + "] (B"+str(i+1)+") at (4,"+str(-i/2.0-1)+") {"+optimized[i]+"};", file=outFile)
   for i in range(len(optimized)):
