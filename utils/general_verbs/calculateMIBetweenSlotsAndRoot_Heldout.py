...
with open(f"results/{__file__}.tsv", "a") as outFile:
 for slot in itos:
    print(slot, (unigramSurprisalPerSlot[slot] - bigramSurprisalPerSlot[slot])/countPerSlot[slot], countPerSlot[slot])
    print("\t".join([str(x) for x in [args.language, slot, (unigramSurprisalPerSlot[slot] - bigramSurprisalPerSlot[slot])/countPerSlot[slot], countPerSlot[slot]]]), file=outFile)
