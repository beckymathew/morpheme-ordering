...
def processVerb(verb, data_):
   ...
   for vb in verb:
      ...
      if "VerbForm=Part" in labels or "VerbForm=Inf" in labels:
          continue
      morphs = turkish_segmenter_coarse.get_abstract_morphemes(labels, only=ONLY)
      fine = turkish_segmenter.get_abstract_morphemes(labels, only=ONLY)
...
for order in orders:
   ...
   results.append((resultingAOC, weights_))
   ...
   elif abs(resultingAOC - mostCorrect) < 1e-5:
      mostCorrectValue.append(weights_)
print(mostCorrectValue)
weights_ = mostCorrectValue[0]
if True:
